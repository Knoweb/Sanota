...
const apiDir = path.join(__dirname, 'src', 'api', 'challenge-enquiry');
    ...
    try {
      ...
      if (files && files['files.supportingFiles']) {
        ...
        for (const file of uploadedFiles) {
          if (!allowedMimeTypes.includes(file.type) && !allowedMimeTypes.includes(file.mimetype)) {
            return ctx.badRequest(\`File type not allowed: \${file.type || file.mimetype}\`);
          }
        }
...
const defaultRouterCode = `import { factories } from '@strapi/strapi';
export default factories.createCoreRouter('api::challenge-enquiry.challenge-enquiry');
`;

const customRouterCode = `export default {
  routes: [
    {
      method: 'POST',
...
export default factories.createCoreService('api::challenge-enquiry.challenge-enquiry');
`;

fs.writeFileSync(path.join(apiDir, 'controllers', 'challenge-enquiry.ts'), controllerCode);
fs.writeFileSync(path.join(apiDir, 'routes', 'challenge-enquiry.ts'), defaultRouterCode);
fs.writeFileSync(path.join(apiDir, 'routes', 'custom-challenge-enquiry.ts'), customRouterCode);
fs.writeFileSync(path.join(apiDir, 'services', 'challenge-enquiry.ts'), serviceCode);

console.log('API files created.');
